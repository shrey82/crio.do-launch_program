...
# ubuntuVersion=`grep 'VERSION_ID=' /etc/os-release | awk 'NR==1{print $1}' | grep -Eo '[+-]?[0-9]+([.][0-9]+)?'`
# ubuntuVersion=${ubuntuVersion|tr -dc '0-9'}
# # ubuntuVersion=`grep "VERSION_ID=" /etc/os-release | awk 'NR==1{print $1}'`
# # ubuntuVersion=`expr substr $ubuntuVersion 13 5`
